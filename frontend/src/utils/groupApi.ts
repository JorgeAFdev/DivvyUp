...
export const getGroupByUserId = async (token: string): Promise<Group[]> => {
    const response = await api.get<Group[]>(`/group/user`, authHeaders(token));
    return response.data;
};

export const getGroupById = async (groupId: string, token: string): Promise<Group> => {
    const response = await api.get<Group>(`/group/${groupId}`, authHeaders(token));
    return response.data;
};

export const createGroup = async (data: GroupInput, token: string): Promise<Group> => {
    const response = await api.post<Group>('/group', data, authHeaders(token));
    return response.data;
};

export const updateGroup = async (groupId: string, data: GroupInput, token: string): Promise<Group> => {
    const response = await api.put<Group>(`/group/${groupId}`, data, authHeaders(token));
    return response.data;
};

export const deleteGroup = async (groupId: string, token: string): Promise<void> => {
    await api.delete(`/group/${groupId}`, authHeaders(token));
};

export const getGroupDetails = async (groupId: string, token: string): Promise<GroupDetails> => {
...
export const getGroupByInviteCode = async (inviteCode: string, token: string): Promise<InviteInfo> => {
    const response = await api.get<InviteInfo>(`/group/join/${inviteCode}`, authHeaders(token));
    return response.data;
};

export const joinGroup = async (inviteCode: string, data: JoinGroupInput, token: string): Promise<Group> => {
    const response = await api.post<Group>(`/group/join/${inviteCode}`, data, authHeaders(token));
    return response.data;
};

export const regenerateInviteCode = async (groupId: string, token: string): Promise<InviteCode> => {
