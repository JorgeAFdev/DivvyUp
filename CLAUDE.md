...
`.env` files are per-workspace (`backend/.env`, `frontend/.env`), not at the root — see README for the full list. Backend needs `MONGO_URL`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL` (the backend's own origin, no `/api/auth` suffix), `CLIENT_URL` (the front origin, used by both CORS layers and Better Auth's `trustedOrigins`), Cloudinary and Resend keys. Frontend needs `VITE_API_URL` and `VITE_SOCKET_URL`.
...
true })`: `updateUser` changes name and picture (email is read-only in the core PR — a body whose
email differs from the session's is a 400), so it is register's rules minus the password, zero
duplication). The 24-hex id validator (`objectId`) is shared from `src/common.ts`.
