...
**Email verification is on but soft** (`emailVerification` in `security/auth.ts`): `sendOnSignUp: true` mails a link through `services/email` (Resend) — its first caller — and `autoSignInAfterVerification` signs the user in after they click. It does **not** gate login: `requireEmailVerification` stays off, deferred to a later child PR. The verification link redirects to the frontend `/email-verified` landing (a public route; the register `signUp.email` call passes it as an absolute `callbackURL`); the email-change flow uses `/email-change` instead. The mail copy lives in `services/authEmails.ts` (`sendVerificationEmail`, `sendChangeEmailConfirmation` — named to match Better Auth's callback keys, so `auth.ts` passes them as shorthand), which `auth.ts` wires in by reference — the config file stays composition, the templates get one home; both compose plain-text mail through `services/email`. `sendVerificationEmail` serves both sign-up **and** the email-change second step (Better Auth reuses it and gives no way to tell them apart), so its copy is address-neutral, not a sign-up welcome. Prod needs `RESEND_FROM` on a **verified** Resend domain — `onboarding@resend.dev` (sandbox) only delivers to the account owner, so real users would silently get nothing.
