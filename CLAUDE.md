...
true })`: `updateUser` changes name and email but not the password, so it is register's rules minus
that one field, zero duplication). The 24-hex id validator (`objectId`) is shared from `src/common.ts`.
