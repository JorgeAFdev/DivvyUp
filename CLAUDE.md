...
- The ban is on *arithmetic*. The frontend's `formatAmount()` (`utils/money.ts`) does call `toFixed(2)`, but only to build a display string out of an amount the backend already rounded at its `.toDecimalPlaces(2).toNumber()` boundary — it never feeds a calculation. `148.60` and `148.6` are the same double, so the trailing zero cannot be stored and has to be produced at render.
...
**The typefaces are self-hosted through Fontsource, not a `<link>` in `index.html`.** `@fontsource/ibm-plex-sans` and `@fontsource/ibm-plex-mono` are imported in `App.tsx` beside `App.css` (latin subsets, weights 400 and 700 only), so the faces ship with the bundle and there is no third-party request. The stacks are `--font-body` (IBM Plex Sans, on `body`) and `--font-mono` (IBM Plex Mono). The mono is **for money**: `balance`, `debt` and `expense` render amounts with it plus `font-variant-numeric: tabular-nums`, so every digit takes one advance width and a figure does not jitter as its value changes. The string itself comes from `formatAmount()` (`utils/money.ts`), the only place an amount becomes text: a fractional amount is padded to cents (`148.6` → `148.60`), a whole one stays bare (`1200`). A column mixing the two therefore does **not** line up on the decimal point — that is the accepted cost of not printing `.00` after every round figure. The `+` is `balance.tsx`'s own and goes on above zero only, so a settled member reads `0€`, not `+0€`. Fallbacks are the metric neighbours, so a face that has not loaded does not reflow the page.
...
The backend runs at `https://divvyup-api.jorgeaf.dev` on a self-hosted **Coolify**, which handles TLS and the reverse proxy and only pulls and runs the GHCR image, so no build happens on the host. Two distinct tokens are involved: a GitHub PAT with `read:packages` on the host, which is what lets Coolify pull the private image, and `COOLIFY_TOKEN`, a Coolify API token held as a repo secret, which the workflow uses to trigger and poll the deploy. Backend env vars: `MONGO_URL` (with `/prod` in the path, **not** empty or MongoDB reads `test`), `BETTER_AUTH_SECRET` (a fresh secret, not the local one), `BETTER_AUTH_URL` (`https://divvyup-api.jorgeaf.dev`), `CLIENT_URL`, `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` (the same Google client also lists `https://divvyup-api.jorgeaf.dev/api/auth/callback/google` as an authorized redirect URI), the three Cloudinary keys, `RESEND_API_KEY`, `RESEND_FROM`.

Host-level detail — the provider, the panel URL, the application UUID, the exact
port hardening and which token lives where — is deliberately **not in the repo**.
It is in `docs/infra-private.md`, which `.gitignore` excludes: none of it is a
secret, but published together it is a map of the server.
