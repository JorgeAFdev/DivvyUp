...
- `pnpm test` in `frontend/` **exits 1**: `icon.test.jsx` is still commented out top to bottom, and jest fails a suite that contains no tests. The live suites are `context/darkModeContext.test.jsx` and `components/header/header.test.jsx` (6 tests, green). Nothing runs jest in CI either — the deploy workflow is path-filtered to `backend/**` and Cloudflare Pages only runs `vite build` — so a frontend test guards intent, not the pipeline. `TODO.md` §4 tracks getting the suite green.
